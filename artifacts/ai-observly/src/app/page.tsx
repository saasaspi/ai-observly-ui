...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden bg-card shadow-sm cursor-pointer" onClick={() => setOpen(!open)}>
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="bg-background border border-border rounded-lg p-4">
          ...
          <div className="flex items-end gap-1.5 h-20">
            {bars.map((h, i) => (
              <div key={i} className="flex-1 rounded-t-sm bg-primary/20 hover:bg-primary/40 transition-colors" style={{ height: `${(h / max) * 100}%` }} />
            ))}
          </div>
    ...
    <PublicLayout>
      ...
      <section id="hero" className="relative pt-28 pb-10 px-6 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          <h1 className="text-5xl md:text-7xl font-bold tracking-tight mb-6 font-outfit text-foreground leading-[1.08]">
            See what your AI is{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">actually costing you</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/signup" className="inline-flex items-center justify-center h-14 px-8 text-lg font-medium rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity w-full sm:w-auto shadow-sm" data-testid="hero-cta">
              Start monitoring free <ArrowRight className="ml-2 w-5 h-5" />
            </Link>
            <a href="#how-it-works" className="inline-flex items-center justify-center h-14 px-8 text-lg font-medium rounded-lg border border-border bg-card hover:bg-muted transition-colors w-full sm:w-auto">
              See how it works
            </a>
          </div>
          <DashboardMockup />
      ...
      <section id="problem" className="py-24 px-6 bg-background">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl md:text-4xl font-bold font-outfit">You shipped an AI feature. Now you're flying blind.</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: AlertCircle, color: "text-red-500", bg: "bg-red-50 border-red-100", title: "Surprise $800 OpenAI bill", desc: "You check your invoice at the end of the month and one customer has been hammering your chatbot — you had no idea until the charge hit." },
              { icon: TrendingDown, color: "text-yellow-600", bg: "bg-yellow-50 border-yellow-100", title: "No idea which feature is burning tokens", desc: "You have four AI features. One of them is unprofitable. You have no way to know which one — so you can't fix it, reprice it, or kill it." },
              { icon: Clock, color: "text-blue-500", bg: "bg-blue-50 border-blue-100", title: "Users complain about slowness — you can't tell why", desc: "Latency spikes when your app hits OpenAI during peak hours. Users churn. You don't have a latency breakdown by model or endpoint to debug it." },
      ...
      <section id="how-it-works" className="py-24 px-6 bg-card border-y border-border">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground text-lg max-w-xl mx-auto">No SDK rewrite. No DevOps. Three steps to clear AI margins.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-12 relative">
            <div className="hidden md:block absolute top-6 left-[calc(16.67%+1rem)] right-[calc(16.67%+1rem)] h-px bg-border" />
            {[
              { num: "1", title: "Generate your API key", desc: "Sign up, get your obs_live_... key in under two minutes. No credit card required for the free tier.", note: "~2 minutes" },
              { num: "2", title: "Drop in one background call", desc: "After each AI response, fire a single fetch() to our endpoint — it never blocks your users and never touches your AI traffic.", note: "~5 lines of code" },
              { num: "3", title: "See cost, latency & profit", desc: "Your dashboard shows every customer and feature: what it costs, what it earns, and whether it's actually worth keeping — in plain dollars.", note: "Live in seconds" },
      ...
      <section id="features" className="py-24 px-6 bg-background">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground text-lg max-w-xl mx-auto">Stop guessing. Start knowing which features and customers are actually profitable.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {[
              { icon: DollarSign, title: "Cost breakdown by customer, feature & endpoint", desc: "See exactly how much each customer costs you in AI, and which feature is the biggest spend — so you can reprice, optimize, or cut the losers." },
              { icon: Zap, title: "Latency & error monitoring", desc: "Track p50/p95 latency per model and endpoint. Get alerted when error rates spike before users start tweeting about it." },
              { icon: Bell, title: "Alerts for spend spikes & failure rates", desc: "Set monthly cost limits or error thresholds per customer. Get a Slack or email alert the moment a customer starts burning through tokens." },
              { icon: BarChart2, title: "Model comparison", desc: "Running GPT-4o alongside Claude? See cost and latency side-by-side so you can pick the right model for each feature — and save money doing it." },
      ...
      <section id="who-its-for" className="py-24 px-6 bg-card border-y border-border">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Indie hackers, solo SaaS founders, and small teams shipping AI features without a dedicated data engineering stack.</p>
          </div>
      ...
      <section id="social-proof" className="py-24 px-6 bg-background">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-muted-foreground text-lg mb-12 max-w-lg mx-auto">Join 143+ founders already on the waitlist — and growing.</p>
          <div className="grid md:grid-cols-3 gap-6 text-left mb-12">
            {[
              { quote: "Finally. I've been staring at my OpenAI bill every month wondering which feature is eating it. This is exactly what I needed.", name: "Sarah K.", role: "Solo founder, AI writing tool", avatar: "SK" },
              { quote: "I repriced my Pro plan within a week of seeing the real cost breakdown. Already back in the black on AI.", name: "Marcus T.", role: "Indie hacker, productivity SaaS", avatar: "MT" },
              { quote: "Langfuse was overkill for what I needed. AI Observly took 10 minutes to set up and showed me the numbers I actually care about.", name: "Priya R.", role: "Founder, AI customer support tool", avatar: "PR" },
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-background border border-border rounded-2xl p-8 flex flex-col">
              ...
              <Link href="/signup" className="block text-center h-12 leading-[3rem] rounded-lg border border-border bg-muted hover:bg-muted/80 font-medium text-foreground transition-colors" data-testid="pricing-free-cta">
                Get started free
              </Link>
            </div>
            <div className="bg-primary text-primary-foreground rounded-2xl p-8 flex flex-col relative overflow-hidden shadow-xl">
              ...
              <Link href="/signup" className="block text-center h-12 leading-[3rem] rounded-lg bg-white text-primary font-semibold hover:bg-white/90 transition-colors" data-testid="pricing-pro-cta">
                Start free trial
              </Link>
            </div>
          </div>
      ...
      <section id="cta" className="py-28 px-6 bg-gradient-to-br from-primary/5 via-background to-indigo-50/40 border-t border-border">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-bold font-outfit mb-4">Stop guessing your AI margins</h2>
          <p className="text-muted-foreground text-lg mb-10 max-w-lg mx-auto">Join the waitlist and be first to know when early access opens. Free tier available from day one.</p>
          <Form {...formBottom}>
            <form onSubmit={formBottom.handleSubmit(handleWaitlist(formBottom))} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto mb-6">
              ...
              <button type="submit" className="h-12 px-6 rounded-md bg-primary text-primary-foreground font-medium hover:opacity-90 transition-opacity whitespace-nowrap shadow-sm" disabled={submitWaitlist.isPending} data-testid="btn-waitlist-submit-bottom">
